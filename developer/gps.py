    ...
    if data[0:6] == '$GPGGA':
        ...
        if int(gps_quality_indictor) > 0:
           ...
           lon_secs = round(float(log_secs_text)*60/10000, 2)
              ...
              speed_over_ground = '0'
              number_of_satellites = '0' 
        gpsTimeSet = gpsTimeSet.encode('utf-8').strip()
    ...
    if data[0:6] == '$GPRMC':
        ...
        if is_number(timeStampStr) == True:
           ...
           if hoursBalance == 0 :
              ...
              # by taking number of days in a month
              # this is expected to happen only at mid night
              # Computation get complicated 
              # So the best option is to wait for few minutes
              # to pass Mid-night transition time. Keep receiving
